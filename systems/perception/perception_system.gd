...
func _setup_vision_shape() -> void:
	...
	var vision_collision = _vision_area.get_node_or_null("VisionPolygon") as CollisionPolygon2D
	if not vision_collision:
		vision_collision = CollisionPolygon2D.new()
		vision_collision.name = "VisionPolygon"
		_vision_area.add_child(vision_collision)
...
func is_in_vision(target: Node2D) -> bool:
	if not _parent_creature:
		return false
	
	var to_target = target.global_position - _parent_creature.global_position
	...
	# 检查角度
	var creature_direction = Vector2.RIGHT.rotated(_parent_creature.rotation)
...
func get_nearest_food() -> Node2D:
	...
	for food in nearby_food.keys():
		var dist = food.global_position.distance_to(_parent_creature.global_position)
		if dist < min_dist:
			min_dist = dist
			nearest = food
...
func get_nearest_threat() -> Node2D:
	...
	for threat in nearby_danger.keys():
		var dist = threat.global_position.distance_to(_parent_creature.global_position)
		if dist < min_dist:
			min_dist = dist
			nearest = threat
